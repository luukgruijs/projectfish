...
module.exports = (app) => {
        ...
        (request, response, next) => {
            ...
            .then((user) => {
                ...
                let msg = {
                    ...
                    "text": `Click the following link to activate your account: ${config.base_url}/#/set-password/${reset_token}/${user._id}`,
    ...
    app.post("/users/forgot_password",
        (request, response, next) => {
            const user = model.User.findOne({ email: request.body.email }, '+password').exec()
            user.then((user) => {
                const existing_user = user;
                const reset_token = token.generateToken();
                existing_user.password = Object.assign({}, {
                    reset_token,
                ...
                .then(() => response.json({message: 'Succesfully updated your account, please check your email'}))
                .catch(next);
            })
        }    
    )
